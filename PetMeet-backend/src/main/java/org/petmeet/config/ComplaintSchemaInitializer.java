...
public class ComplaintSchemaInitializer implements CommandLineRunner {
    ...
    public void run(String... args) {
        try {
            ...
            log.error("Failed to ensure cms_complaint schema. Please run SQL migration: sql/migration_extend_cms_complaint_feedback.sql", e);
        }
    }
    ...
    private void ensureComplaintSchema() {
        if (!tableExists("cms_complaint")) {
            jdbcTemplate.execute("""
                    ...
                    """);
            log.info("Created table cms_complaint");
            return;
        }
